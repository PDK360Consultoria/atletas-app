...
async function syncUserActivities(db, user, opts) {
  ...
  const attach = db.prepare(`UPDATE activities SET
    ...
    WHERE id = ?`);
  const findByExternal = db.prepare('SELECT id, workout_type, intervals_json FROM activities WHERE user_id = ? AND external_id = ?');
  const updateRow = db.prepare('UPDATE activities SET workout_type = COALESCE(workout_type, ?), intervals_json = COALESCE(?, intervals_json) WHERE id = ?');
  ...
  for (const act of items) {
    ...
    if (matched) {
      attach.run(String(act.id), workoutTypeLabel, avgHr, maxHr, elevation, matched.id);
      count += 1;
      continue;
    }
    ...
    // Backfill workout type / interval laps for rows that don't have them
    // yet — covers both a freshly-inserted row and one synced before this
    // feature existed.
    const missingData = !existing || existing.workout_type == null || existing.intervals_json == null;
    ...
    if (finalLabel || intervalsJson) {
      updateRow.run(finalLabel, intervalsJson, targetId);
    }
  }
...
module.exports = {
  ...
  looksLikeIntervals, normalizeIntervalLaps, syncUserActivities,
};
