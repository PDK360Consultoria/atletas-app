...
// Friendly relative timestamp for feed posts/comments ("agora", "há 5 min",
// "há 3h", "ontem") — falls back to fmtDate once it's more than a week old,
// since "há 340h" stops being useful information at that point. `iso` is a
// UTC "datetime('now')" string from SQLite (no timezone suffix), so it's
// parsed as UTC explicitly rather than relying on Date's local-time default
// for a bare "YYYY-MM-DD HH:MM:SS" string.
...
function icon(name, uid) {
  ...
  const paths = {
    ...
    camera: `<path d="M4 8.2a2 2 0 0 1 2-2h1.1l.9-1.6a1.6 1.6 0 0 1 1.4-.8h5.2a1.6 1.6 0 0 1 1.4.8l.9 1.6H18a2 2 0 0 1 2 2v9.4a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z" fill="url(#${g})"/><circle cx="12" cy="13" r="3.6" fill="#15171A" opacity=".28"/><circle cx="12" cy="13" r="2.1" fill="url(#${g})"/><circle cx="16.3" cy="9.3" r=".7" fill="#15171A" opacity=".3"/>`,
    close: `<path d="M6 6l12 12M18 6L6 18" stroke="url(#${g})" stroke-width="2.4" fill="none" stroke-linecap="round"/>`,
  };
  return `<svg class="icon-svg" viewBox="0 0 24 24" aria-hidden="true">${defs}${paths[name] || ''}</svg>`;
}

module.exports = { secToPace, fmtClock, fmtDate, timeAgo, esc, parseClock, renderMarkdownLite, icon };
