...
// hand. Matches by same user, same day, distance within 150m and duration
// within 90s, keeping the Strava-linked row (so it can't be duplicated
// again) and folding the manual row's workout_type/notes/analysis/blocks
// into it before deleting it. Safe to run on every boot: once a pair is
// merged there's nothing left for it to match.
{
  ...
  // external_id), so its title is what the athlete actually typed — keep that
  // over Strava's generic/translated default name.
  ...
  for (const [, list] of byUser) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (Math.abs(a.distance_km - b.distance_km) > 0.15) continue;
        if (a.duration_sec != null && b.duration_sec != null && Math.abs(a.duration_sec - b.duration_sec) > 90) continue;
